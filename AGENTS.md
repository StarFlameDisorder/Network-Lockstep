...
- **管线化输入**：客户端在途输入（已发送帧 - 服务端已消费帧）达 `MAX_INPUT_PIPELINE` 暂停发送 → 服务端队列有界，tick 漂移不再积压；服务端广播为**限速事件驱动**（距上帧 ≥ 帧间隔 **或** 输入队列积压 ≥ `SERVER_BROADCAST_DRAIN_QUEUE` 即广播——平滑时 30fps，客户端领先时快速跟进降延迟）。
- **断线重连**：客户端收包超时 3s 检测 + TCP/KCP 双通道统一状态；服务端统一广播权威快照 + 全量补帧（所有客户端从同一快照点重置，根治位置漂移）。
- **Desync 检测**：客户端按已执行帧里程碑（每 30 帧）上报世界哈希 → 服务端**只比对同帧哈希** → 不一致广播 DesyncNoticeMessage。
- **调参入口**：代码常量统一 `Assets/Scripts/Core/GameConstants.cs`；服务端资产可编辑项在 ServerConfig。
