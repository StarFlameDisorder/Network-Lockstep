...
### 正确性（帧同步命根子）
- **Desync 检测闭环**（原 TODO P3，升级）✅ 已解决（2026-08-03）：客户端每 30 帧上报世界哈希（HashReportMessage），服务端跨客户端同帧（±1 帧容差）比对，不一致时广播 DesyncNoticeMessage（含分歧帧号）到所有客户端并打日志。已落地：proto 消息 + 服务端 RoomManager.ReceiveHashReport 比对 + 客户端 TryReportHash/ReceiveDesyncNotice。**假阳性修复（2026-08-04）**：原 ±1 容差在"每帧都在变的世界上"必然误报（帧差1=状态本就不同，且旧版上报帧号用"已收到帧"而哈希实际是"已执行帧"的状态）；改为客户端按**已执行帧里程碑**（30 的倍数）上报（帧号=哈希实际对应的 LastExecutedFrameId）+ 服务端**只比对同帧哈希**（帧号不同跳过等对齐）
...
- **长期运行延迟加大** ✅ 已解决（2026-08-04）：根因是 tick 漂移——客户端/服务端各自浮点累加器驱动 30fps 无时间同步，客户端略快 → 服务端 `InputQueue` 无上限积压 → 输入等待时间线性增长。已落地：① 服务端广播改**限速事件驱动 + 积压排空**（输入全部就绪 +（距上帧 ≥ 帧间隔 或 队列积压 ≥ `SERVER_BROADCAST_DRAIN_QUEUE=2`）即广播——平滑 30fps，客户端领先时快速跟进降延迟）② 客户端输入**管线化**（在途输入 = 已发送帧 - 服务端已消费帧，达 `MAX_INPUT_PIPELINE=3` 暂停发送 → 队列有界，延迟不再无界增长）③ `CheckInputQueueBacklog`（1s 节流告警）保留作诊断
